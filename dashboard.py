...
# Загрузка данных (с подавлением предупреждения о смешанных типах)
@st.cache_data
def load_data():
    df = pd.read_csv('chat_processed.csv', parse_dates=['datetime'], low_memory=False)
    return df
...
if 'type' in df.columns:
    # Исключаем служебные типы, которые могут не иметь текста
    all_types = sorted(df['type'].unique())
    default_types = [t for t in all_types if t != 'service']  # обычно исключаем service
    ...
    # Если колонки 'type' нет, то пропускаем этот фильтр
    selected_types = None
...
# 7. Опционально: фильтр по типу медиа (если есть колонка 'media_type' или подобная)
...
if not filtered_df.empty:
    ...
    # Доп. метрика: средний интервал между сообщениями (в секундах)
    sorted_df = filtered_df.sort_values('datetime')
    time_diff = sorted_df['datetime'].diff().dt.total_seconds()
    # Убираем большие паузы (более 1 дня), чтобы не искажать среднее
...
if not filtered_df.empty:
    # Устанавливаем datetime как индекс для resample
...
# Таблица с данными (разворачиваемая)
